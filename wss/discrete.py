...
class REINFORCE(base.REINFORCE):
    ...
    def compute_loss(self, states, actions, rewards):
        ...
        for t in range(len(rewards)):
            ...
            action_probs, termination_probs = self.policy_network(state)
            action_log_prob = tf.math.log(action_probs[0, int(action[0])])
            termination_log_prob = tf.math.log(termination_probs[0, int(action[1])])
            
            loss -= action_log_prob * reward
            loss -= termination_log_prob * reward
        return loss