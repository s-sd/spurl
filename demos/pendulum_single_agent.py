...
reinforce = REINFORCE(env, policy_network, scale=0.5, artificial_truncation=2048)
...
for meta_trial in range(meta_trials):
    ...
    reinforce = train(reinforce, trials=2, episodes_per_trial=8, epochs_per_trial=2, batch_size=64, verbose=True)    
...
save_environment_render(rendering_env, algorithm=reinforce, save_path=os.path.join(temp_path, 'pendulum_trajectory'), deterministic=True, artificial_truncation=2048)
