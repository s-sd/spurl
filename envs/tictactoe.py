...
# all classes adapted from https://github.com/davidADSP/SIMPLE
class Player():
...
class TicTacToeEnv(gym.Env):
    ...
    def render(self, mode='human', close=False, verbose = True):
        ...
        print(' '.join([x.symbol for x in self.board[:self.grid_length]]))
        print(' '.join([x.symbol for x in self.board[self.grid_length:self.grid_length*2]]))
        print(' '.join([x.symbol for x in self.board[(self.grid_length*2):(self.grid_length*3)]]))
